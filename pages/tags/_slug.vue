    ...
    <div class="w-full bg-semi-black">
        <div class="max-w-7xl mx-auto px-6 pt-28 md:pt-20">
                ...
                <div class="flex flex-row justify-between">
                    <div class=" py-8 ">
                        <nuxt-link :to="path" class="text-lg md:text-xl text-semi-gray hover:underline pb-1.5 md:pb-20 md:mb-10"> 
                            <div class="flex">
                                <Prev class="md: pt-1 pr-2"/> GDSC CAU Tech Stack
                            </div>
                        </nuxt-link>
                        <div class="text-semi-white text-4xl md:text-6xl font-bold pb-0 md:pb-1.5 md:pt-10 uppercase">{{tag.name}}</div>
                        <div class=" text-lg md:text-xl text-semi-gray pb-1.5 md:pb-3 md:pr-20 ">{{tag.description}}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="block md:hidden">
                ...
                <div class="text-semi-white text-2xl md:text-3xl font-medium flex justify-center  pb-0 md:pb-1.5">{{tag.name}}</div>
                <div class="text-semi-white px-0 md:px-20 pb-4 flex justify-center text-center custom-text">{{tag.description}}</div>
            </div>
        </div>
...
export default {
    async asyncData({ $content, params }) {
        ...
        const proArticles = await $content('conferences')
            .where({ tag: { $contains: tagName } })
            .sortBy('desc')
            .fetch();
        return { tag, tagName, proArticles }
    },
    data() {
        return {
        path: '/',
