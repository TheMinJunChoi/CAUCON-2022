<template>
<div class="accordion" id="accordionExample5">
  <div class="accordion-item border border-gray-200 bg-white">
    <h2 class="accordion-header mb-0" id="headingOne5">
      <button class="accordion-button relative flex items-center w-full py-6 px-5 bg-white
      text-sm md:text-base text-gray-800 text-left border-0 rounded-none transition focus:outline-none
      " type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne5" aria-expanded="true"
        aria-controls="collapseOne5">
        컨퍼런스를 참관하고 싶은데, 참여 자격이 있나요?
      </button>
    </h2>
    <div id="collapseOne5" class="accordion-collapse collapse show" aria-labelledby="headingOne5">
      <div class="accordion-body py-6 px-5 text-sm">
        참여 자격은 따로 존재하지 않습니다! GDSC 중앙대 챕터에 관심이 있는 분이라면 누구든 참관하실 수 있어요. GDSC 멤버이실 필요도, 중앙대생이실 필요도, 대학생이실 필요도 없어요.
      </div>
    </div>
  </div>
  <div class="accordion-item bg-white border border-gray-200">
    <h2 class="accordion-header mb-0" id="headingTwo5">
      <button class="
        accordion-button
        collapsed
        relative
        flex
        items-center
        w-full
        py-6
        px-5
        text-sm md:text-base text-gray-800 text-left
        bg-white
        border-0
        rounded-none
        transition
        focus:outline-none
      " type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo5" aria-expanded="false"
        aria-controls="collapseTwo5">
        참여 신청은 어떻게 할 수 있나요?
      </button>
    </h2>
    <div id="collapseTwo5" class="accordion-collapse collapse" aria-labelledby="headingTwo5">
      <div class="accordion-body py-6 px-5 text-sm">
        6월 28일부터 Festa 플랫폼을 통해 무료 티켓을 판매해요. 티켓을 구매하시면 타임테이블, 컨퍼런스 장소 등 자세한 안내 메일을 보내드릴 예정이에요.
      </div>
    </div>
  </div>
  <div class="accordion-item bg-white border border-gray-200">
    <h2 class="accordion-header mb-0" id="headingThree5">
      <button class="
        accordion-button
        collapsed
        relative
        flex
        items-center
        w-full
        py-6
        px-5
        text-sm md:text-base text-gray-800 text-left
        bg-white
        border-0
        rounded-none
        transition
        focus:outline-none
      " type="button" data-bs-toggle="collapse" data-bs-target="#collapseThree5" aria-expanded="false"
        aria-controls="collapseThree5">
        어떤 세션들이 준비되어 있나요?
      </button>
    </h2>
    <div id="collapseThree5" class="accordion-collapse collapse" aria-labelledby="headingThree5">
      <div class="accordion-body py-6 px-5 text-sm">
        GDSC CAUCON 2022는 GDSC 중앙대 챕터의 활동보고 세션과 메타(Meta) 소프트웨어 엔지니어의 키노트, GDSC 중앙대 챕터 멤버들의 프로젝트 발표 세션들, GDSC 중앙대 2기 모집 설명 세션으로 구성되어 있어요.
      </div>
    </div>
  </div>
</div>
</template>

<script>
export default {

}
</script>

<style>
</style>